...
//
//	@id				Initialize
func (s *server) Initialize(otelServiceName string, entrypointLogFilePath string, organizationId, regionId, boxId *string) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		err := s.initTelemetry(ctx.Request.Context(), otelServiceName, entrypointLogFilePath, organizationId, regionId, boxId)
		if err != nil {
			ctx.AbortWithError(http.StatusBadRequest, err)
			return
		}
