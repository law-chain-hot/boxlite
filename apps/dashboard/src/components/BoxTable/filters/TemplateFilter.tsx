...
export function TemplateFilterIndicator({ value, onFilterChange, templates, isLoading }: TemplateFilterProps) {
  return (
    <div className="flex items-center h-6 gap-0.5 rounded-sm border border-border bg-muted/80 hover:bg-muted/50 text-sm">
      <Popover>
        ...
        <PopoverContent className="p-0 w-[240px]" align="start">
          <TemplateFilter value={value} onFilterChange={onFilterChange} templates={templates} isLoading={isLoading} />
        </PopoverContent>
      </Popover>
...
export function TemplateFilter({ value, onFilterChange, templates, isLoading }: TemplateFilterProps) {
  ...
  const filteredTemplates = templates.filter((template) => {
    ...
    return [template.displayName, template.name, template.artifactRef, template.description]
      .filter(Boolean)
      .some((field) => field?.toLowerCase().includes(search))
  })

  const handleSelect = (templateName: string) => {
    const newValue = value.includes(templateName)
      ? value.filter((name) => name !== templateName)
      : [...value, templateName]
            ...
            <CommandGroup>
              {filteredTemplates.map((template) => (
                <CommandCheckboxItem
                  ...
                  checked={value.includes(template.name)}
                >
                  <div className="min-w-0 space-y-0.5">
                    <div className="truncate text-sm">
                      {template.displayName || getTemplateDisplayName(template.name)}
                    </div>
                    <div className="truncate text-xs text-muted-foreground">{template.name}</div>
                  </div>
                </CommandCheckboxItem>
              ))}
            </CommandGroup>
