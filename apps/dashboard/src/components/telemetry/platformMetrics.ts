...
const PLATFORM_METRIC_GROUPS: PlatformMetricGroupDefinition[] = [
  {
    ...
    title: 'Request path',
    description:
      'Start here when admins feel the product is slow: API server/client latency is the user-facing signal.',
    watchFirst: true,
    convertToGiB: false,
    matches: (metricName) => metricName.startsWith('http.server.') || metricName.startsWith('http.client.'),
  },
  {
    key: 'dependencies',
    title: 'Dependency pressure',
    description:
      'Database duration, pending requests, and connection pressure explain whether the API is waiting below it.',
    watchFirst: true,
    convertToGiB: false,
  ...
  },
  {
    ...
    description: 'Event loop delay, utilization, and GC duration show whether Node.js is too busy to schedule work.',
    watchFirst: true,
    convertToGiB: false,
    matches: (metricName) => metricName.startsWith('nodejs.eventloop.') || metricName.startsWith('v8js.gc.'),
  },
  {
    key: 'memory',
  ...
  },
  {
    ...
    description: 'Lifecycle and scheduled job durations show whether control-plane maintenance work is slow or stuck.',
    watchFirst: false,
    convertToGiB: false,
  ...
  },
  {
    ...
    description: 'Raw platform metrics that are visible but do not yet map to a first-pass troubleshooting group.',
    watchFirst: false,
    convertToGiB: false,
    matches: () => true,
  },
...
const DISPLAY_NAMES: Record<string, string> = {
  ...
  'v8js.gc.duration': 'V8 GC duration',
  'http.server.duration': 'HTTP server duration',
  'http.client.duration': 'HTTP client duration',
