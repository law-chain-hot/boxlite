...
import { useCreateTemplateMutation } from '@/hooks/mutations/useCreateTemplateMutation'
...
export const CreateTemplateDialog = ({ className, ref }: { className?: string; ref?: Ref<{ open: () => void }> }) => {
  const [open, setOpen] = useState(false)

  const { availableRegions: regions, loadingAvailableRegions: loadingRegions } = useRegions()
  const { selectedOrganization } = useSelectedOrganization()
  const { reset: resetCreateTemplateMutation, ...createTemplateMutation } = useCreateTemplateMutation()
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      if (!selectedOrganization?.id) {
        toast.error('Select an organization to create a template.')
        return
      }
      ...
      try {
        ...
        toast.success(`Creating template ${value.name.trim()}`)
        setOpen(false)
      } catch (error) {
  ...
  const resetState = useCallback(() => {
    ...
    resetCreateTemplateMutation()
  }, [resetCreateTemplateMutation, form])
  ...
  return (
    ...
    >
      <DialogTrigger asChild>
        <Button variant="default" size="sm" className="ml-auto" title="Create Template">
      ...
      <DialogContent className={className}>
        <DialogHeader>
          ...
          <DialogDescription>
            Register a new template to be used for spinning up boxes in your organization.
          </DialogDescription>
        </DialogHeader>
              ...
              {(field) => {
                ...
                return (
                  <Field data-invalid={isInvalid}>
                    ...
                    />
                    <FieldDescription>
                      The name you will use in your client app (SDK, CLI) to reference the template.
                    </FieldDescription>
            ...
            <form.Field name="regionId">
              {(field) => (
                <Field>
                  ...
                  <FieldDescription>
                    The region where the template will be available. If not specified, your organization's default
                    region will be used.
                  </FieldDescription>
                </Field>
              )}
            ...
            <form.Field name="entrypoint">
              {(field) => (
                <Field>
                  ...
                  />
                  <FieldDescription>
                    Ensure that the entrypoint is a long running command. If not provided, or if the template does not
