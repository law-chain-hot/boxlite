...
export function BillingPanel({
  ...
  const filtered = useMemo(() => {
    ...
    return invoices.filter((invoice) =>
      [invoice.issuingDate, invoice.type, statusLabel(invoice.paymentStatus), usd(invoice.totalAmountCents)].some(
  ...
  return (
    <div className="flex flex-col gap-9">
      ...
      <div>
        ...
        {paged.length === 0 ? (
          <div className="px-[2px] py-10 text-center font-mono text-[12px] text-muted-foreground">
            no receipts found
          </div>
        ) : (
          paged.map((invoice) => {
