...
import { useCreateSavedImageMutation } from '@/hooks/mutations/useCreateSavedImageMutation'
...
export const CreateSavedImageDialog = ({ className, ref }: { className?: string; ref?: Ref<{ open: () => void }> }) => {
  const [open, setOpen] = useState(false)

  const { availableRegions: regions, loadingAvailableRegions: loadingRegions } = useRegions()
  const { selectedOrganization } = useSelectedOrganization()
  const { reset: resetCreateSavedImageMutation, ...createSavedImageMutation } = useCreateSavedImageMutation()
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        await createSavedImageMutation.mutateAsync({
          savedImage: {
  ...
  const resetState = useCallback(() => {
    ...
    resetCreateSavedImageMutation()
  }, [resetCreateSavedImageMutation, form])
    ...
    >
      ...
      <DialogContent className={className}>
        <DialogHeader>
          ...
          <DialogDescription>Register a reusable image for spinning up boxes in your organization.</DialogDescription>
        </DialogHeader>
        <ScrollArea fade="mask" className="h-[500px] overflow-auto -mx-5">
          <form
            ref={formRef}
            id="create-saved-image-form"
          ...
          >
              ...
              {(field) => {
                ...
                return (
                  <Field data-invalid={isInvalid}>
                    ...
                    />
                    <FieldDescription>
                      The name you will use in your client app (SDK, CLI) to reference this image.
                    </FieldDescription>
            ...
            <form.Field name="imageName">
              {(field) => {
                ...
                return (
                  <Field data-invalid={isInvalid}>
                    <FieldLabel htmlFor={field.name}>Source Image</FieldLabel>
                    <Input
                      aria-invalid={isInvalid}
                      id={field.name}
                      name={field.name}
