...
function makeSandbox(
  ...
  return {
    ...
    boxId: overrides.boxId ?? 'public-box-1',
...
function makeOrganization(overrides: Partial<{ id: string; name: string; personal: boolean; createdBy: string }> = {}) {
  return {
    ...
    name: overrides.name ?? 'Alice Personal',
...
function buildService(stubs: {
  ...
  usersByIds?: ReturnType<typeof makeUser>[]
  ...
  const userService = {
    ...
    findByIds: jest.fn().mockResolvedValue(stubs.usersByIds ?? stubs.users ?? []),
  } as any
  ...
  const sandboxes = stubs.sandboxes ?? []
  const stateCounts = sandboxes.reduce<Record<string, number>>((acc, sandbox) => {
    acc[sandbox.state] = (acc[sandbox.state] ?? 0) + 1
    return acc
  }, {})

  const sandboxRepository = {
    find: jest.fn().mockResolvedValue(sandboxes),
  ...
  return new AdminOverviewService(userService, runnerService, sandboxRepository, organizationService)
}
...
describe('AdminOverviewService', () => {
  ...
  describe('getOverview', () => {
    it('counts users, active boxes, and runner states correctly', async () => {
      ...
      expect(result.boxes).toEqual({ total: 2, byState: { [SandboxState.STARTED]: 2 } })
  ...
  describe('listBoxes', () => {
    it('maps sandbox entity fields and owner metadata to AdminBoxItemDto', async () => {
      const service = buildService({
        usersByIds: [makeUser({ id: 'usr-1', email: 'owner@example.com', name: 'Owner' })],
        organizations: [makeOrganization({ id: 'org-1', name: 'Owner Personal', personal: true, createdBy: 'usr-1' })],
        sandboxes: [
          makeSandbox({
            id: 'sb-1',
            boxId: 'box-public-1',
      ...
      expect(result[0].boxId).toBe('box-public-1')
