...
export class AdminS3ObjectReader {
  ...
  async listRelatedObjects(
    ...
    if (!region || buckets.length === 0 || !accessKey || !secretKey) {
      return {
        objects: [],
        status: {
          source: 's3',
          state: 'not_configured',
          message: 'Admin S3 lookup requires region, bucket, and credentials',
          count: 0,
        },
      }
    }
    ...
    const client = new S3Client({
      ...(endpoint ? { endpoint: endpoint.startsWith('http') ? endpoint : `http://${endpoint}` } : {}),
      region,
