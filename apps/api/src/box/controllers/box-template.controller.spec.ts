...
describe('BoxController template creation contract', () => {
  it('allows non-admin users to create a box from a template with resource overrides', async () => {
    ...
    const controller = new BoxController({} as any, boxService as any, redisMock() as any)
    ...
    expect(boxService.createFromTemplate).toHaveBeenCalledWith(createBoxDto, organization)
  })
})
