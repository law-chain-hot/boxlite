...
export class BoxTemplateService {
  ...
  private async handleBoxCreatedEvent(event: BoxCreatedEvent) {
    if (!event.box.template) {
      return
    }

    // Update once per minute at most
    if (!(await this.redisLockProvider.lock(`template:${event.box.template}:update-last-used`, 60))) {
      return
    }

    const template = await this.getBoxTemplateByName(event.box.template, event.box.organizationId)
    template.lastUsedAt = event.box.createdAt
    await this.boxTemplateRepository.save(template)
  }
