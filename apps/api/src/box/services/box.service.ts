...
export class BoxService {
  ...
  async createForWarmPool(warmPoolItem: WarmPool): Promise<Box> {
    ...
    box.template = warmPoolItem.template
    ...
    const template = await this.boxTemplateRepository.findOne({
      where: [
        { organizationId: box.organizationId, name: box.template, state: BoxTemplateState.ACTIVE },
        { general: true, name: box.template, state: BoxTemplateState.ACTIVE },
      ],
    })
    if (!template) {
      throw new BadRequestError(`BoxTemplate ${box.template} not found while creating warm pool sandbox`)
    }
  ...
  private async createFromBoxTemplate(
    ...
    try {
      ...
      if (createBoxDto.cpu !== undefined) {
        cpu = createBoxDto.cpu
      }
      if (createBoxDto.memory !== undefined) {
        mem = createBoxDto.memory
      }
      if (createBoxDto.disk !== undefined) {
        disk = createBoxDto.disk
      }
      if (createBoxDto.gpu !== undefined) {
  ...
  async createFromTemplate(createBoxDto: CreateBoxDto, organization: Organization): Promise<BoxDto> {
    const requestedTemplate = createBoxDto.templateId?.trim() || this.configService.getOrThrow('defaultTemplate')
    const templateId = resolveSystemTemplateName(requestedTemplate) ?? requestedTemplate
    return this.createFromBoxTemplate(createBoxDto, organization, templateId)
  }
