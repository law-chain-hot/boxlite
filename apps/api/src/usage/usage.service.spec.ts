...
class FakeTransactionManager {
  ...
  async findOne(
    entity: typeof UsagePeriod | typeof Box,
    opts: { where: Partial<UsagePeriod> | Partial<Box> },
  ): Promise<UsagePeriod | Box | null> {
    if (entity === Box) {
      return this.periods.boxes.get((opts.where as Partial<Box>).id ?? '') ?? null
    }

    return this.periods.findOne(opts as { where: Partial<UsagePeriod> })
  }

  async save(
    entity: typeof UsagePeriod | typeof UsagePeriodArchive,
    rowOrRows: UsagePeriod | UsagePeriod[] | UsagePeriodArchive[],
  ): Promise<UsagePeriod | UsagePeriod[] | UsagePeriodArchive[]> {
    if (entity === UsagePeriod) {
      return this.periods.save(rowOrRows as UsagePeriod | UsagePeriod[])
    }

    const rows = rowOrRows as UsagePeriodArchive[]
    rows.forEach((row) => {
...
describe('UsageService', () => {
  ...
  beforeEach(() => {
    ...
    service = new UsageService(periods as never, archives as never, locks as never)
  })
  ...
  it('archives closed periods and removes them from the active table', async () => {
    ...
    periods.manager.transaction.mockClear()
  ...
  it('propagates usage ledger failures so the caller can roll back the box update', async () => {
    periods.failNextSave = new Error('database unavailable')

    await expect(
      service.applyTransition(
        makeBox(BoxState.STARTED, BoxDesiredState.STARTED),
        BoxState.STARTED,
        BoxDesiredState.STARTED,
        new Date(),
      ),
