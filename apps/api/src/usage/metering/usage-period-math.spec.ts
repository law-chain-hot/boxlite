...
import { aggregateUsagePeriods, planUsageTransition, usagePeriodKind, UsagePeriodLike } from './usage-period-math'
...
describe('usagePeriodKind', () => {
  ...
  it('maps terminal non-billable states to gone', () => {
    expect(usagePeriodKind(BoxState.ERROR, BoxDesiredState.STARTED)).toBe('gone')
    expect(usagePeriodKind(BoxState.ARCHIVED, BoxDesiredState.DESTROYED)).toBe('gone')
  })
})
