...
describe('BillingController', () => {
  ...
  it('delegates automatic top-up, top-up, usage, tiers, invoices, and placeholder URLs', async () => {
    ...
    walletService.voidInvoice.mockResolvedValue(undefined)
    ...
    await expect(
      controller.setAutomaticTopUp('org-1', { thresholdAmount: 10, targetAmount: 30 }),
    ...
    await expect(controller.voidInvoice('org-1', 'inv-1')).resolves.toBeUndefined()
    await expect(controller.getOrganizationBillingPortalUrl('org-1')).resolves.toBe(
      'http://localhost/billing/portal/org-1',
    )
    await expect(controller.getOrganizationCheckoutUrl('org-1')).resolves.toBe(
      'http://localhost/billing/checkout/org-1',
    )
  })
})
